...
% Time parameters
OS = 0.45;  % reduced from 45% to 5%, more reasonable
Ts = 7;     % from spec
ln_os = log(OS);
...
% Kp_c2 = 2 / DC_gain;
C_outer_aug = C_outer * 1/s *2/(DC_gain);
%C2aug_num = C2_aug.Numerator{1}; C2aug_den = C2_aug.Denominator{1};
% 1/(4*DC_gain);
G_inner = C_inner * P_inner / (1+ C_inner * P_inner);
P_combined = G_inner * G_2;
G_outer = C_outer_aug * P_combined / (1+ C_outer_aug * P_combined);
